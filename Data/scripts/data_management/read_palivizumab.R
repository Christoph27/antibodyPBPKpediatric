...
getPalivizumabPK <- function() {
  ...
  # ---- Adult (Boeckh2001) ----
  adult_f <- file.path("..", "..", "rawData", "palivizumab", "Boeckh2001_palivizumab_Adults.xlsx", fsep = .Platform$file.sep)
  ...
  adult <- adult[, .(time_days, concDoseNorm1mgkg, sdDoseNorm1mgkg, Ab, group)]
  adult[, dosingCycle := 1]

  # ---- Subramanian1998 (pediatric 10 & 15 mg/kg) ----
  sub_f <- file.path("..", "..", "rawData", "palivizumab", "Subramanian1998_Palivizumab_Ped.xlsx", fsep = .Platform$file.sep)
  ...
  sub_all <- rbindlist(list(sub15[, .(time_days, concDoseNorm1mgkg, sdDoseNorm1mgkg, Ab, group, dosingCycle)],
                             sub10[, .(time_days, concDoseNorm1mgkg, sdDoseNorm1mgkg, Ab, group, dosingCycle)]), use.names = TRUE, fill = TRUE)
  

  # ---- SaezLlorens2004 (pediatric 15 mg/kg) ----
  saez_f <- file.path("..", "..", "rawData", "palivizumab", "SaezLlorens2004_Palivizumab_Ped.xlsx", fsep = .Platform$file.sep)
  ...
  # ---- combine ----
  combinedData <- rbindlist(list(adult, sub_all, saez_dt[, .(time_days, concDoseNorm1mgkg, sdDoseNorm1mgkg, Ab, group, dosingCycle)]), use.names = TRUE, fill = TRUE)
